...
function csrf($bucket)
{
    $data=array("corsConfiguration"=>array(array("allowedOrigins"=>array("http://*", "https://*"),
        "allowedMethods"=>array("GET","HEAD","DELETE","POST","PUT"),
        "allowedHeaders"=>array("*"),
        "allowedExposeHeaders"=>array("ETag",
            "Content-Length",
    ...
    $credentials = array("ak" => "6bb013b74f43441aa2961130a550837d", "sk" => "114576782dd448b89c8e4b2b2db551be");
    $httpMethod = "PUT";
    $path = "/";
    $params = array("cors" => "");
    $timestamp = date_create();
    $headers = array(
        "Host" => $bucket.".bcebos.com",
        "x-bce-date" => $timestamp->format("Y-m-d\TH:i:s\Z"),
        "Content-Length" => strlen($data_json),
        "Content-Type" => "application/json; charset=utf-8"
    );
    $options = array(SignOption::TIMESTAMP => $timestamp, SignOption::EXPIRATION_IN_SECONDS => SignOption::MAX_EXPIRATION_IN_SECONDS);
    $ret = $signer->sign($credentials, $httpMethod, $path, $headers, $params, $options);
    echo $ret;
    echo "\n";
//$timestamp->setTimezone(new DateTimeZone('PRC'));
    $ch = curl_init();
    $header[] = "Host: ".$bucket.".gz.bcebos.com";
    $header[] = "x-bce-date: " . $timestamp->format("Y-m-d\TH:i:s\Z");
    ...
    $header[] = "Accept:";
    curl_setopt($ch, CURLOPT_URL, "http://mireco-test.gz.bcebos.com/?cors"); //定义请求地址
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json); //定义提交的数据
